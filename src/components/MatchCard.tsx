...
export const MatchCard: React.FC<MatchCardProps> = ({
  ...
  const handleClaim = async () => {
    setResolving(true);
    try {
      // Mark candidate and source report as resolved
      await fetch(`/api/reports/${candidate.report.id}`, {
        method: "PATCH",
      ...
      if (sourceReport.id) {
        await fetch(`/api/reports/${sourceReport.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status: "resolved" }),
        });
        ...
        <div className="flex items-center gap-2">
          {isClaimed ? (
            ...
            <button
              onClick={handleClaim}
              disabled={resolving}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-600/20 transition active:scale-95 disabled:opacity-50"
            >
              <Check className="w-3.5 h-3.5" />
              {resolving ? "Resolving..." : "Confirm & Claim Match"}
            </button>
          )}
